...
(0, node_test_1.describe)('GrandExchangeDatabaseDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.RunescapeApisSDK({
    ...
    (0, node_test_1.test)('direct-load-grand_exchange_database', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-grand_exchange_database', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-grand_exchange_database', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-grand_exchange_database', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.RUNESCAPE_APIS_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.RunescapeApisSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['RUNESCAPE_APIS_TEST_GRAND_EXCHANGE_DATABASE_ENTID'];
    ...
    const client = new __1.RunescapeApisSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=GrandExchangeDatabaseDirect.test.js.map