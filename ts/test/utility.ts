...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.RUNESCAPEAPIS_TEST_LIVE ||
    'TRUE' === process.env.RUNESCAPEAPIS_TEST_OVERRIDE
  ) {
  ...
  m.RUNESCAPEAPIS_TEST_EXPLAIN = process.env.RUNESCAPEAPIS_TEST_EXPLAIN || m.RUNESCAPEAPIS_TEST_EXPLAIN
