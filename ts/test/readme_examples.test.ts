...
//     (\`new RunescapeApisSDK(...)\` / \`RunescapeApisSDK.test(...)\` become
//     \`RunescapeApisSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'RunescapeApisSDK'
...
const TEST_SEED = {"entity":{"grand_exchange_database":{"test01":{"id":"test01"}},"old_school_grand_exchange":{"test01":{"id":"test01"}},"player_ranking":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new RunescapeApisSDK(...)\` and any
// \`RunescapeApisSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RunescapeApisSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RunescapeApisSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
