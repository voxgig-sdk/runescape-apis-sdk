...
describe('GrandExchangeDatabaseDirect', async () => {
  ...
  test('direct-load-grand_exchange_database', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-grand_exchange_database', setup.live)) return
  ...
  test('direct-list-grand_exchange_database', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-grand_exchange_database', setup.live)) return
    const { client, calls } = setup
