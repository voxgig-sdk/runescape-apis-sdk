...
func TestOldSchoolGrandExchangeEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.OldSchoolGrandExchange(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.OldSchoolGrandExchange(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		oldSchoolGrandExchangeRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.old_school_grand_exchange")))
...
func old_school_grand_exchangeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("RUNESCAPE_APIS_TEST_OLD_SCHOOL_GRAND_EXCHANGE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["RUNESCAPE_APIS_TEST_OLD_SCHOOL_GRAND_EXCHANGE_ENTID"])
