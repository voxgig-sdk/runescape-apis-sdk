...
func TestPlayerRankingEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.PlayerRanking(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.PlayerRanking(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		playerRankingRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.player_ranking")))
...
func player_rankingBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("RUNESCAPE_APIS_TEST_PLAYER_RANKING_ENTID")
	...
	idmapResolved := core.ToMapAny(env["RUNESCAPE_APIS_TEST_PLAYER_RANKING_ENTID"])
