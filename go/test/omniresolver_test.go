...
import (
	...
	omni "github.com/voxgig-sdk/runescape-apis-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RunescapeApisError); is {
				out := map[string]any{
					"name":    "RunescapeApisError",
...
func omniCtx(arg any, client *sdk.RunescapeApisSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RunescapeApisSDK); is {
			client = live
