...
class RunescapeApisTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            RunescapeApisTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = RunescapeApisTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = RunescapeApisTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = RunescapeApisTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = RunescapeApisTestRunner.getenv(key)
        ...
        explain = RunescapeApisTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if RunescapeApisTestRunner._test_control is not None:
            return RunescapeApisTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                RunescapeApisTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            RunescapeApisTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = RunescapeApisTestRunner.load_test_control()
...
def load_env_local():
    RunescapeApisTestRunner.load_env_local()
...
def entity_list_to_data(lst):
    return RunescapeApisTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return RunescapeApisTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return RunescapeApisTestRunner.load_test_control()


def live_client_options():
    return RunescapeApisTestRunner.live_client_options()


def live_delay_ms():
    return RunescapeApisTestRunner.live_delay_ms()
