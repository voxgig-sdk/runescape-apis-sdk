...
module RunescapeApisUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = RunescapeApisContext.new({}, nil)
    end
    op = ctx.op || RunescapeApisOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || RunescapeApisResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(RunescapeApisError) ? err.msg : err.to_s
    ...
    sdk_err = RunescapeApisError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(RunescapeApisError)
