...
// RunescapeApis SDK feature test
//
...
class FtCtrl extends RunescapeApisControl
{
...
class FtClient
{
    ...
    private RunescapeApisUtility $utility;

    public function __construct(RunescapeApisUtility $utility, array $options)
    {
    ...
    public function get_utility(): RunescapeApisUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RunescapeApisContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RunescapeApisContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RunescapeApisContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RunescapeApisConfig::make_feature($name);
    ...
    private static function buildUrl(RunescapeApisSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RunescapeApisError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RunescapeApisResponse($fetched);
        }
    ...
    private function fail(RunescapeApisContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RunescapeApisContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
