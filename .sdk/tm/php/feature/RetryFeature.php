...
// RunescapeApis SDK retry feature
...
class RunescapeApisRetryFeature extends RunescapeApisBaseFeature
{
    ...
    public function init(RunescapeApisContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (RunescapeApisContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(RunescapeApisContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(RunescapeApisContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
