...
class RunescapeApisSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = RunescapeApisHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = RunescapeApisHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!RunescapeApisFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, RunescapeApisFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return RunescapeApisUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = RunescapeApisHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = RunescapeApisHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = RunescapeApisHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new RunescapeApisError($op . "_allow",
                "RunescapeApisSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = RunescapeApisHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new RunescapeApisError("graphql_error",
                "RunescapeApisSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new RunescapeApisSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
