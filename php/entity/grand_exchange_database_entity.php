...
// RunescapeApis SDK GrandExchangeDatabase entity
...
class GrandExchangeDatabaseEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "grand_exchange_database";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new GrandExchangeDatabaseEntity($this->_client, $opts);
    }

    /**
     * @param GrandExchangeDatabase|array $args GrandExchangeDatabase data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = RunescapeApisHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GrandExchangeDatabase|array The current GrandExchangeDatabase data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GrandExchangeDatabase fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = RunescapeApisHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GrandExchangeDatabase fields).
     */
    ...
    /**
     * Load a single GrandExchangeDatabase.
     *
     * @param GrandExchangeDatabaseLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GrandExchangeDatabaseLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GrandExchangeDatabase|array The loaded GrandExchangeDatabase as an assoc-array at the
     *   SDK boundary; throws RunescapeApisError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = RunescapeApisHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List GrandExchangeDatabase items matching the given filter.
     *
     * @param GrandExchangeDatabaseListMatch|array|null $reqmatch Match filter (any subset
     *   of GrandExchangeDatabase fields) as an assoc-array; GrandExchangeDatabaseListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GrandExchangeDatabase[]|array A list of GrandExchangeDatabase items as assoc-arrays at
     *   the SDK boundary; throws RunescapeApisError on failure (item-5 convention).
     */
