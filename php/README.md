...
The PHP SDK for the RunescapeApis API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->GrandExchangeDatabase(null)->list(null, null);
...
```php
[$result, $err] = $client->GrandExchangeDatabase(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->RunescapeApis(null)->load(
    ["id" => "test01"], null
);
