...
class GrandExchangeDatabaseDirectTest extends TestCase
{
    public function test_direct_list_grand_exchange_database(): void
    {
        $setup = grand_exchange_database_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-grand_exchange_database", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
    ...
    public function test_direct_load_grand_exchange_database(): void
    {
        $setup = grand_exchange_database_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-grand_exchange_database", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
