...
describe("GrandExchangeDatabaseDirect", function()
  it("should direct-list-grand_exchange_database", function()
    local setup = grand_exchange_database_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-grand_exchange_database", setup.live and "live" or "unit")
  ...
  it("should direct-load-grand_exchange_database", function()
    local setup = grand_exchange_database_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-grand_exchange_database", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function grand_exchange_database_direct_setup(mockres)
