# RunescapeApis Lua SDK



The Lua SDK for the RunescapeApis API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:GrandExchangeDatabase()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/runescape-apis-sdk/releases)),
...
Check `err` first (it is non-`nil` on failure), then use `value`:
    ...
    local grand_exchange_database, err = client:GrandExchangeDatabase():load()
    if err then error(err) end
...
```lua
local grand_exchange_database, err = client:GrandExchangeDatabase():load({ item_id = 1 })
```
...
```
lua/
├── runescape-apis_sdk.lua    -- Main SDK module
...
The main module (`runescape-apis_sdk`) exports the SDK constructor
