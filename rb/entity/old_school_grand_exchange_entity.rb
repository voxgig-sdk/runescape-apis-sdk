...
class OldSchoolGrandExchangeEntity
  def initialize(client, entopts = nil)
    ...
    @_name = "old_school_grand_exchange"
    @_client = client
  ...
  def make
    opts = @_entopts.dup
    OldSchoolGrandExchangeEntity.new(@_client, opts)
  end
  ...
  #
  # @param reqmatch [OldSchoolGrandExchangeListMatch, Hash, nil] match filter (any subset of
  #   OldSchoolGrandExchange fields); defaults to nil, treated as an empty match that lists all.
