...
class PlayerRankingDirectTest < Minitest::Test
  def test_direct_list_player_ranking
    setup = player_ranking_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-player_ranking", setup[:live] ? "live" : "unit")
    if _should_skip
...
def player_ranking_direct_setup(mockres)
