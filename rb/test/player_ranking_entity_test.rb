...
class PlayerRankingEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.PlayerRanking(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.PlayerRanking(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set RUNESCAPE_APIS_TEST_PLAYER_RANKING_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    player_ranking_ref01_list_result = player_ranking_ref01_ent.list(player_ranking_ref01_match, nil)
...
def player_ranking_basic_setup(extra)
  ...
  entid_env_raw = ENV["RUNESCAPE_APIS_TEST_PLAYER_RANKING_ENTID"]
