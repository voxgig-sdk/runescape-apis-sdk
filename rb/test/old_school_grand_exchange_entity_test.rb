...
class OldSchoolGrandExchangeEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = RunescapeApisSDK.test(seed, nil)
    seen = base.OldSchoolGrandExchange(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.OldSchoolGrandExchange(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = old_school_grand_exchange_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "old_school_grand_exchange." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set RUNESCAPE_APIS_TEST_OLD_SCHOOL_GRAND_EXCHANGE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    old_school_grand_exchange_ref01_data = nil
    if old_school_grand_exchange_ref01_data_raw.length > 0
...
def old_school_grand_exchange_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "old_school_grand_exchange", "OldSchoolGrandExchangeTestData.json")
  ...
  entid_env_raw = ENV["RUNESCAPE_APIS_TEST_OLD_SCHOOL_GRAND_EXCHANGE_ENTID"]
