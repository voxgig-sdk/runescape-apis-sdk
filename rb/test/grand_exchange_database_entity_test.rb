...
class GrandExchangeDatabaseEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.GrandExchangeDatabase(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.GrandExchangeDatabase(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set RUNESCAPE_APIS_TEST_GRAND_EXCHANGE_DATABASE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    grand_exchange_database_ref01_data_dt0_load_result = Helpers.to_map(grand_exchange_database_ref01_data_dt0_loaded.respond_to?(:data_get) ? grand_exchange_database_ref01_data_dt0_loaded.data_get : grand_exchange_database_ref01_data_dt0_loaded)
...
def grand_exchange_database_basic_setup(extra)
  ...
  entid_env_raw = ENV["RUNESCAPE_APIS_TEST_GRAND_EXCHANGE_DATABASE_ENTID"]
