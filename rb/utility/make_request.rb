...
module RunescapeApisUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RunescapeApisResponse.new({})
    ...
    if fetch_err
      ...
      response = RunescapeApisResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RunescapeApisResponse.new(fetched)
    else
