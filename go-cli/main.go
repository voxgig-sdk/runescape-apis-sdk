// Command RunescapeApis-cli is a CLI + REPL for the RunescapeApis SDK,
...
const prompt = "runescape-apis"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "grand_exchange_database old_school_grand_exchange player_ranking"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      RUNESCAPE_APIS_APIKEY (api key), RUNESCAPE_APIS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: RUNESCAPE_APIS_APIKEY carries the API key and
	// RUNESCAPE_APIS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("RUNESCAPE_APIS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("RUNESCAPE_APIS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewRunescapeApisSDK(opts)
