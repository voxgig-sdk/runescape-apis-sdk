...
class TestOldSchoolGrandExchangeEntity:

    def test_should_create_instance(self):
        testsdk = RunescapeApisSDK.test(None, None)
        ent = testsdk.OldSchoolGrandExchange(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.OldSchoolGrandExchange(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.OldSchoolGrandExchange(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _old_school_grand_exchange_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "old_school_grand_exchange." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set RUNESCAPE_APIS_TEST_OLD_SCHOOL_GRAND_EXCHANGE_ENTID JSON to run live")
        client = setup["client"]
        ...
        old_school_grand_exchange_ref01_data = None
        if len(old_school_grand_exchange_ref01_data_raw) > 0:
            old_school_grand_exchange_ref01_data = helpers.to_map(old_school_grand_exchange_ref01_data_raw[0][1])

        # LIST
        old_school_grand_exchange_ref01_ent = client.OldSchoolGrandExchange(None)
        old_school_grand_exchange_ref01_match = {}

        old_school_grand_exchange_ref01_list_result = old_school_grand_exchange_ref01_ent.list(old_school_grand_exchange_ref01_match, None)
        assert isinstance(old_school_grand_exchange_ref01_list_result, list)



def _old_school_grand_exchange_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/old_school_grand_exchange/OldSchoolGrandExchangeTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("RUNESCAPE_APIS_TEST_OLD_SCHOOL_GRAND_EXCHANGE_ENTID"))
