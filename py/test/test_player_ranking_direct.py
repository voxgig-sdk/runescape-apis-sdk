...
class TestPlayerRankingDirect:

    def test_should_direct_list_player_ranking(self):
        setup = _player_ranking_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-player_ranking", "live" if setup["live"] else "unit")
        if _skip:
...
def _player_ranking_direct_setup(mockres):
