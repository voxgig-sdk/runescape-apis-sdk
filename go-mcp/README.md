...
MCP server exposing the RunescapeApis SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `runescape-apis_list` | `entity`, optional `query` map | First page of records as JSON |
| `runescape-apis_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `runescape-apis_list` and
`runescape-apis_load` tools become available in new sessions.
