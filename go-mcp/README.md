...
[MCP](https://modelcontextprotocol.io) server exposing the RunescapeApis SDK as
two agent tools — `runescape-apis_list` and `runescape-apis_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/runescape-apis-mcp)
make build
...
claude mcp add --scope user runescape-apis \
  -- /absolute/path/to/runescape-apis-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./runescape-apis-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// runescape-apis_list: first page of records
{ "entity": "grand_exchange_database" }
{ "entity": "grand_exchange_database", "query": { } }

// runescape-apis_load: one record by id
{ "entity": "grand_exchange_database", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/runescape-apis-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user runescape-apis \
     -- "$PWD"/dist/*/runescape-apis-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `runescape-apis_list` and `runescape-apis_load` tools now appear
   in new sessions. Ask the agent to *"list grand_exchange_database using runescape-apis"*
   and it calls `runescape-apis_list` with `{"entity":"grand_exchange_database"}`.
...
### Call the `runescape-apis_list` tool
...
### Call the `runescape-apis_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `runescape-apis_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `runescape-apis_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./runescape-apis-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"runescape-apis_load","arguments":{"entity":"grand_exchange_database","query":{"id":1}}}}'
```
