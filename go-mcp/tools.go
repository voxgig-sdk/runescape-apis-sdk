...
type Args struct {
	Entity string         `json:"entity" jsonschema:"grand_exchange_database | old_school_grand_exchange | player_ranking"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.RunescapeApisSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "runescape-apis_list",
		Description: "List records from RunescapeApis. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "runescape-apis_load",
		Description: "Load a single record from RunescapeApis. " +
...
func runOp(client *sdk.RunescapeApisSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.RunescapeApisSDK, name string) (sdk.RunescapeApisEntity, error) {
	switch strings.ToLower(name) {
	case "grand_exchange_database":
		return client.GrandExchangeDatabase(nil), nil
	case "old_school_grand_exchange":
		return client.OldSchoolGrandExchange(nil), nil
	case "player_ranking":
		return client.PlayerRanking(nil), nil
